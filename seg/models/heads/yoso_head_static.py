...
class StaticSepConvAtten(nn.Module):
    """🔥 完全静态的高性能深度可分离卷积机制 - 无任何注意力"""
    def __init__(self, hidden_dim, num_proposals, conv_kernel_size_1d):
        super(StaticSepConvAtten, self).__init__()
        self.hidden_dim = hidden_dim
        self.num_proposals = num_proposals
        self.kernel_size = min(max(conv_kernel_size_1d, 11), 21)
        
        # 🔥 优化的多尺度静态卷积核
        # 使用不同大小的卷积核捕获多尺度特征
        kernel_sizes = [self.kernel_size, self.kernel_size + 4, self.kernel_size + 8]
        self.depth_conv_weights = nn.ParameterList([
            nn.Parameter(torch.Tensor(num_proposals, 1, ks)) for ks in kernel_sizes
        ])
        self.point_conv_weights = nn.ParameterList([
            nn.Parameter(torch.Tensor(num_proposals, num_proposals, 1)) for _ in range(3)
        ])
        
        # 🔥 去掉prompt training
        #self.kernel_importance = nn.Parameter(torch.ones(3))
        ...
        # 🔥 特征增强网络 - 替代注意力机制
        self.feature_enhancer_conv = nn.Sequential(
        ...
        # 初始化
        for i in range(3):
            nn.init.normal_(self.depth_conv_weights[i], mean=0, std=0.01)
            nn.init.normal_(self.point_conv_weights[i], mean=0, std=0.01)
    ...
    def forward(self, query, value):
        ...
        # 🔥 特征增强
        enhanced_value = self.feature_enhancer(value)
        
        # 🔥 多尺度卷积处理
        multi_scale_outputs = []
        enhanced_value_unsqueezed = enhanced_value.unsqueeze(1)
        
        for i in range(3):
            res = []
            for b in range(B):
                # 🔥 使用不同大小的卷积核
                kernel_size = int(self.depth_conv_weights[i].size(-1))
                padding = kernel_size // 2  # 使用整数除法
                
                # 深度卷积
                out = F.relu(F.conv1d(
                    input=enhanced_value_unsqueezed[b], 
                    weight=self.depth_conv_weights[i].unsqueeze(0).expand(B, -1, -1, -1)[b], 
                    groups=N, 
                    padding=padding
                ))
                # 点卷积
                out = F.conv1d(
                    input=out, 
                    weight=self.point_conv_weights[i].unsqueeze(0).expand(B, -1, -1, -1)[b], 
                    padding=0
                )
                res.append(out)
            
            multi_scale_outputs.append(torch.cat(res, dim=0))
        
        # 🔥 多尺度融合
        fused_output = torch.zeros_like(multi_scale_outputs[0])
        for i, output in enumerate(multi_scale_outputs):
            fused_output += self.scale_weights[i] * output
        
        # 🔥 特征增强 - 替代注意力机制
        fused_output_transposed = fused_output.transpose(1, 2)  # [B, C, N]
        
        # 特征增强
        enhanced_features = self.feature_enhancer_conv(fused_output_transposed)
        
        # 特征选择（类似门控机制，但不是注意力）
        selection_weights = self.feature_selector(fused_output_transposed)
        enhanced_features = enhanced_features * selection_weights
        
        # 转回原始维度
        point_out = enhanced_features.transpose(1, 2)  # [B, N, C]
        point_out = self.norm(point_out)
        
        # 🔥 残差连接 + 门控机制
        gate = torch.sigmoid(point_out)
        return gate * point_out + (1 - gate) * value
...
class CrossAttenHead(nn.Module):
    ...
    def __init__(self,
        ...
        if use_kernel_updator:
            ...
            # 🔥 使用完全静态的高性能注意力机制
            self.f_atten = StaticSepConvAtten(self.feat_channels, self.num_proposals, conv_kernel_size_1d)
            self.f_dropout = nn.Dropout(0.0)
            self.f_atten_norm = nn.LayerNorm(self.hidden_dim * self.conv_kernel_size_2d**2)
            self.k_atten = StaticSepConvAtten(self.feat_channels, self.num_proposals, conv_kernel_size_1d)
        ...
        if self.use_self_attn:
            ...
            # 🔥 使用完全静态的高性能注意力机制
            self.s_atten = StaticSepConvAtten(self.feat_channels, self.num_proposals, conv_kernel_size_1d)
    ...
    def forward(self, features, proposal_kernels, mask_preds, self_attn_mask=None):
        ...
        if self.use_self_attn:
            ...
            # 🔥 使用完全静态的高性能卷积机制 - 无注意力
            k_tmp = self.s_atten(k, k)
            k = k + self.s_dropout(k_tmp)
            k = self.s_atten_norm(k)
